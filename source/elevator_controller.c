...
void door_opening () { //FUNKSJON TIL DOOR OPENING
    hardware_command_movement(HARDWARE_MOVEMENT_STOP); //STOPP HEISEN
    hardware_command_order_light(current_floor.floor, HARDWARE_ORDER_INSIDE, 0); //VI FJERNER ORDER_LIGHTS (ELLER PUTTE DEN I REMOVER order?)
    hardware_command_order_light (current_floor.floor, HARDWARE_ORDER_UP, 0);
    hardware_command_order_light (current_floor.floor, HARDWARE_ORDER_DOWN, 0);
